...
import { getSchoolBySubdomain } from './api/school.api'; // Adjust path as needed
import { getCoursesBySchool } from './api/course.api'; // Adjust path as needed
...
interface Course {
  _id: string;
  courseName: string;
  // Add other fields from your API if needed (e.g., description, fee)
}
...
const MarketingPage: React.FC = () => {
    ...
    if (subdomain) {
      const fetchSchoolData = async () => {
        try {
          ...
          const dbname = 'gamersclub'; // Adjust if dynamic
          const coursesResponse = await getCoursesBySchool(schoolId, dbname);

          // Extract course names (adjust path if structure differs, e.g., coursesResponse.courses)
          const fetchedCourses = (coursesResponse?.data?.courses || []).map((course: Course) => course.courseName || 'Unnamed Course');

          setSchoolData((prev) => ({ ...prev, coursesOffered: fetchedCourses }));
        } catch (err) {
          console.error('Error fetching data:', err);
          setError('Failed to load school or course data. Please try again.');
        } finally {
          setLoading(false);
        }
      };
  ...
  // Dummy course details (unchanged; consider merging API data for dynamic values if available)
  const courseDetails = {
  ...
  if (loading) {
    return <div className="flex justify-center items-center min-h-screen">Loading school data...</div>;
  }
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
              {schoolData.coursesOffered.map((course, index) => {
                const details = courseDetails[course as keyof typeof courseDetails] || courseDetails['Full Stack Web Development']; // Type assertion for lookup
