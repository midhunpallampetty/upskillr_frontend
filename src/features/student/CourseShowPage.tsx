...
const ExamComponent = ({ exam, onSubmit, onCancel }: { exam: ExamType | null; onSubmit: (passed: boolean, score: number, total: number) => void; onCancel: () => void }) => {
  ...
  const calculateScore = () => {
    ...
    (exam?.questions || []).forEach((q) => {
      if (!q || !q.correctAnswer || !q.options?.length) return;  // Skip invalid questions
    ...
    >
      ...
      {questions.length === 0 ? (
        ...
        questions.map((q, index) => (
          <div key={q?._id || index} className="mb-6 p-4 border border-gray-200 rounded-lg">  {/* Fallback key if _id is missing */}
            <h3 className="font-semibold mb-3">{index + 1}. {q?.questionText || 'Question text missing'}</h3>
            <div className="grid gap-2">
              {(q?.options || []).length > 0 ? (  // Check for options before mapping
                ...
                <p className="text-yellow-600 text-center">No options available for this question.</p>
              )}
            </div>
          </div>
        ))
