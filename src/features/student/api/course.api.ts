...
export const fetchCoursesBySchool = async (
  schoolName: string
): Promise<{
  success: boolean;
  courses?: any[];
  ...
  try {
    const res = await courseAxios.post('/courses', { schoolName });
    return {
      success: true,
...
export const fetchPurchasedCourses = async (studentId: string) => {
  try {
    const { data } = await courseAxios.get(`/course/school-info/${studentId}`);
    return data;
  } catch (error) {
...
// Fetch course details (updated endpoint to match original: /${schoolName}/course/${courseId})
...
export const fetchStudentProgress = async (
  ...
  try {
    const { data } = await courseAxios.get(
      `/${schoolName}/courses/${courseId}/progress`,
      { params: { studentId } }
    );
    return data;
  } catch (error) {
...
export const updateCertificate = async (
  ...
  try {
    const { data } = await courseAxios.put(
      `/${schoolName}/courses/${courseId}/certificates`,
...
// New helper for initiating checkout (based on component's payment POST)
