...
export const registerStudent = async (formData: RegisterData & { schoolName: string }) => {
  try {
...
export const loginStudent = async (
  ...
  try {
    const res = await studentAxios.post(`/login`, formData);
    return res.data;
  } catch (err: any) {
  ...
  // student.api.ts
 export const verifyStudentOtp = (email: string, otp: string, schoolName: string) =>
  studentAxios.post('/verify-otp', { email, otp, schoolName });

export const sendStudentResetLink = async (email: string, schoolName: string): Promise<string> => {
  const response = await studentAxios.post(`/forgot-password`, { email, schoolName });
...
export const getStudentById = async (id: string, schoolName: string) => {
  const response = await studentAxios.get(`/student/${id}`, { params: { schoolName } });
  return response.data.student;
};


 export const updateStudentById = async (id: string, payload: any, schoolName: string) => {
  return await studentAxios.put(`/students/${id}`, { ...payload, schoolName });
};
